...
import socket, traceback, psutil, sys


LPORT: int = int(sys.argv[1]) if len(sys.argv) > 1 else 58999
LHOST: str =  "0.0.0.0"
CON_LIMIT: int = 3
...
def is_connections_limit_exceeded(ip: str, limit: int) -> bool:
    ...
    Return True if the current host already has more connections than limit.
...
if __name__ == "__main__":
    ...
    while True:
        ...
        try:
            thread = ClientThread(args = [client_socket])
            thread.start()
        except Exception:
